<template>
  <a-drawer
    ...
    <a-form :form="form">
     ...
     <a-form-item label='总重量' v-bind="formItemLayout">
        <a-input  v-decorator="['currentWeight']"/>
     </a-form-item>
...
export default {
  ...
  methods: {
    ...
    setFormValues ({...warehousedetail}) {
      ...
      let fields = ['warehouseDetailName','rawmaterialName','currentWeight','warnWeight']
    ...
    handleSubmit () {
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          this.$put('warehousedetail/clear', {
            ...warehousedetail
          }).then((r) => {
            this.loading = false
            this.$emit('success')
